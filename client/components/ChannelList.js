...
export default class ChannelList extends Component {
  ...
  render () {
    ...
    const messageAmt = (channelNum) => this.state.messages.filter(a => a.channelId === channelNum).length
    return (
      <ul>
        <li>
          <NavLink to={RANDOM_CHANNEL} activeClassName="active">
            <span># really_random</span>
            <span className="badge">{messages.filter(a => a.channelId === 1).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={GENERAL_CHANNEL} activeClassName="active">
            <span># generally_speaking</span>
            <span className="badge">{messages.filter(a => a.channelId === 2).length}</span>
          </NavLink>
        </li>
        <li>
          <NavLink to={DOGS_CHANNEL} activeClassName="active">
            <span># dogs_of_fullstack</span>
            <span className="badge">{messages.filter(a => a.channelId === 3).length}</span>
          </NavLink>
        </li>
        <li>
